  ...
  return (
    <group ref={groupRef}>
      <Float speed={2.5} rotationIntensity={0.15} floatIntensity={0.6}>
        ...
        <Image url="/photos/hero.jpeg" transparent radius={0.3} position={[0, 0, 0.05]} scale={[3.0, 3.8]} />
        ...
        <RoundedBox args={[3.3, 4.1, 0.01]} radius={0.4} smoothness={8} position={[0, 0, -0.1]}>
          <meshBasicMaterial color="#38BDF8" transparent opacity={0.1} blending={THREE.AdditiveBlending}/>
        </RoundedBox>

        {/* Outer ambient glow */}
        <mesh position={[0, 0, -0.4]}>
          <planeGeometry args={[6, 7]} />
          <meshBasicMaterial color="#3B82F6" transparent opacity={0.06} blending={THREE.AdditiveBlending} depthWrite={false} />
        </mesh>
      </Float>
    </group>
  )
...
export default function ProfileScene() {
  return (
    <Canvas camera={{ position:[0, 0, 7.5], fov:45 }}
      ...
      <pointLight position={[4, -4, -4]} intensity={1.5} color="#38BDF8"/>
      <Suspense fallback={null}>
        <ElegantAvatar/>
      </Suspense>
      <Sparkles count={55} scale={8} size={2.5} speed={0.4} color="#3B82F6" opacity={0.4}/>
    </Canvas>
  )
}
